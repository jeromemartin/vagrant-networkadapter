      ...
      class NetworkAdapterRenameHyperV
        ...
        def call(env)
          ...
          if @enabled
              ...
              if !adap.has_key?(:vmname) or !adap.has_key?(:newname)
          ...
          if @enabled
            network_adapters_added.each do |adap|
              if !adap.has_key?(:switchname) or !adap.has_key?(:name)
                env[:ui].error "vagrant-networkadapter (add): invalid network adapters configuration: missing 'name' or 'switchname'."
              else
        ...
        def call(env)
          ...
          if @enabled
              ...
              if !adap.has_key?(:switchname)
                env[:ui].error "vagrant-networkadapter (change): invalid network adapters configuration: missing 'switchname'."
              else
                switchname = adap[:switchname].encode("ISO-8859-1")
				if adap.has_key?(:name) 
					name = adap[:name].encode("ISO-8859-1")
